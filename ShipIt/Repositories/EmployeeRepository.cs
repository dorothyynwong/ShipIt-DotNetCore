...
namespace ShipIt.Repositories
{
    public interface IEmployeeRepository
    {
        ...
        EmployeeDataModel GetEmployeeById(int em_id);
    ...
    public class EmployeeRepository : RepositoryBase, IEmployeeRepository
    {
        ...
        public EmployeeDataModel GetEmployeeById(int em_id)
        {
            string sql = "SELECT em_id, name, w_id, role, ext FROM em where em_id = @em_id";
            var parameter = new NpgsqlParameter("@em_id", em_id);
            string noProductWithIdErrorMessage = string.Format("No employee found with ID: {0}", em_id);
            return base.RunSingleGetQuery(sql, reader => new EmployeeDataModel(reader), noProductWithIdErrorMessage, parameter);
        }
